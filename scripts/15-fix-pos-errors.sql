-- Fix POS Interface Errors
-- Comprehensive script to resolve database issues causing payment processing errors

-- 1. Ensure all required columns exist in products table
...
-- 3. Ensure all required columns exist in transactions table
...
-- 4. Ensure all required columns exist in transaction_items table
ALTER TABLE transaction_items ADD COLUMN IF NOT EXISTS price_includes_gst BOOLEAN DEFAULT false;

-- 5. Ensure customers table exists and has required columns
...
-- 6. Ensure loyalty_transactions table exists
...
-- 7. Fix the get_products_with_stock function
...
-- 8. Fix the get_last_bill_number function
...
-- 9. Create function to get unique brands
...
DO $$
BEGIN
    RAISE NOTICE 'POS Interface database fixes completed successfully!';
